...
With respect to CaMKII, concentrations of CaMKIIα subunits have been reported to range between 5 and 100 $\mu\text{M}$ in neuronal dendrites [@lee2009Activation; @feng2011Quantitative; @otmakhov2012Measuring]. NMDARs are typically found at approximately 10 to 100 receptors per synapse (Lisman et al., 2012; Petralia et al., 2009). In the present model, the concentrations of CaMKII and NMDARs are positioned at the lower end of these reported ranges, primarily due to the relatively large spine volume modeled. Specifically, we include 290 CaMKII subunits (equivalent to 2.36 $\mu\text{M}$) and 30 NMDARs (0.099 $\mu\text{M}$). Although the concentration of CaMKII in this model is on the lower end of reported values, this choice remains valid given the larger spine volume incorporated in the model, which dilutes the concentration of CaMKII while maintaining a reasonable estimate for its overall abundance [REFERENCE]. Furthermore, the number of NMDARs in the model is within the reported range, reinforcing the biological relevance of the simulation. The limitations of these parameter choices and potential future directions for refining this model are further discussed in the Future Directions section.

For comparison, if we take experimental estimates which are on the lower end to compare appropriaely, we take CaMKII subunits concentrations around 5 μM and NMDAR concentrations of approximately 0.17 μM (Lisman et al., 2012; Petralia et al., 2009), yielding a literature-based ratio of roughly 29:1. Thus, while the absolute concentrations in the model are somewhat lower, the relative ratio between CaMKII and NMDARs is broadly consistent with experimental observations.


other things binding to calcium. calcium influx more like an action potential. synaptic input would be utliple calcium inputs. calcium uncaging 

Similarly, CaM concentrations reported in the literature range from 0.1 to ~10 $\mu\text{M}$ in dendritic spines [@zeng2010Effect]. The model uses a CaM concentration of 0.98 $\mu\text{M}$, which is within this range.

when it comes to camkii subunits, concentrations of camkii alpha subunits have been reported to range from arounf 5 - 100 $\mu\text{M}$ at neuronal dendrites [REFERENCES]. and nmdars are found at arounf 10 to 100 receptors per synapse [references]. our concentrations of camkii and nmdars concentrations are on the lower end of the scale (due to the higher volume), 290 subunits (2.36 $\mu\text{M}$) camkii subunits and 30 NMDARs (0.099 $\mu\text{M}$). giving a model ratio of about 29 camkii subunits per 3 nmdar (29:3). if we take the lower end of Experimental estimates place CaMKII concentrations [REFERENCE CHECK HOLONENZYME OR SUBUNITS] in dendritic spines around 5 $\mu\text{M}$, with NMDAR concentrations reported at approximately 0.17 μM (Lisman et al., 2012; Petralia et al., 2009), yielding a literature-based ratio of roughly 29:1. 

100 μM alpha subunits https://www.sciencedirect.com/science/article/abs/pii/S0006899311015782 We estimate that the percentage of spine CaMKII held active by binding to the NMDAR is ~ 0.2%. 

71, 46, and 103 μM, subunits https://www.sciencedirect.com/science/article/abs/pii/S016502701100570X

camkii alpha subunits ∼10–100 μM https://www.nature.com/articles/nature07842


Experimental estimates place CaMKII concentrations [REFERENCE CHECK HOLONENZYME OR SUBUNITS] in dendritic spines around 5 $\mu\text{M}$, with NMDAR concentrations reported at approximately 0.17 μM (Lisman et al., 2012; Petralia et al., 2009), yielding a literature-based ratio of roughly 29:1. 

RATIO I USE OF CAMKII/NMDAR is same ratio as bartol et al., 

In the current model, CaMKII SUBUNITS is present at 2.36 $\mu\text{M}$, and NMDARs at 0.099 $\mu\text{M}$, giving a model ratio of about 24:1. 

This close correspondence demonstrates that the functional relationship between CaMKII and NMDARs is preserved, even if the absolute concentrations are slightly reduced. PP1 concentrations in neurons are typically reported between 1 μM and 10 μM (Cohen, 2002), with CaMKII concentrations around 5 $\mu\text{M}$, yielding a literature-based ratio of approximately 5:1. In the model, PP1 is present at 0.039 $\mu\text{M}$, and CaMKII at 2.36 $\mu\text{M}$, resulting in a model ratio of about 60:1. While this higher CaMKII:PP1 ratio may exaggerate the kinase dominance. Higher PP1 concentrations should be considered in future iterations for a more accurate representation of the CaMKII:PP1 ratio, though the model remains useful for exploring activation dynamics and regulation under varying conditions.
...
|-----------------|---------------------------------|-----------------------------|
|
$\mathrm{Ca^{2+}}$ binds to CaM in a stepwise manner, with higher affinity for forming CaM_Ca3 and CaM_Ca4 states [REFERENCE]. | Rules #1 - #4 (@fig-tbl-1)| Yes, see @fig-calcium-binding
|
CaMKII autophosphorylation at T286 is promoted once CaMKII is bound to NMDARs [REFERENCE].  | Rules #5, #6, #8, #12 (@fig-tbl-1)| Yes, see |
|
CaM binds CaMKII only in its fully saturated state (CaM_Ca4), stabilizing CaMKII in its open conformation [REFERENCE]. | Rule #6 (@fig-tbl-1) |  Yes, see |
|
Open CaMKII binds to NMDARs, anchoring the kinase at the membrane  [REFERENCE].| Rule #12 (@fig-tbl-1) | Yes, see |
|
CaMKII undergoes autophosphorylation at T286 regardless of CaM binding, though this phosphorylation is enhanced by CaM binding [REFERENCE].| Rule #8 (@fig-tbl-1)| Yes, see | 
|
CaMKII undergoes autophosphorylation at T306 following T286 phosphorylation [REFERENCE]. | Rules #8, #9 (@fig-tbl-1)| Yes, see |
|
CaMKII subunits can be found to be T306-phosphorylated and in a closed conformation [REFERENCE]. | Rules #5, #10 (@fig-tbl-1)| Yes, see |
|
PP1 dephosphorylates CaMKII at T286 and T306, reversing its activation and enabling cycling between open and closed states. [REFERENCE]. | Rules #10, #11 (@fig-tbl-1)| Unclear, see |
