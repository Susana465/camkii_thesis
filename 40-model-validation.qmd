...
The table below relates to @fig-tbl-1; where there are expected behaviours resulting from addition of coded reactions. 


|Expected Result| Associated Reaction Rules | Observed in Models |
|-----------------|---------------------------------|-----------------------------|
|
CaMKII undergoes autophosphorylation at T286 regardless of CaM binding, though this phosphorylation is enhanced by CaM binding [REFERENCE]. | Rules  (@fig-tbl-1)| Yes, see @fig-calcium-binding
|
...
4. **T306-phosphorylated CaMKII can adopt a closed conformation, which eventually this leads to decreased binding to nmdars.** wedo not code this into the model as a direct conditionconsistent with structural studies suggesting reversion to an "autoinhibited" state following disengagement from activators.

--
6. **if camkii is t306 p, eventually this leads to decreased binding to nmdars**

4. **Only Open CaMKII binds to NMDARs, anchoring the enzyme at the postsynaptic membrane.** This statement encapsulates 


Taken together, these outcomes confirm that the model produces not only direct, rule-driven behaviour, but also a range of higher-order dynamics that reflect the biological system it aims to replicate. This capacity for emergent behaviour strengthens confidence in the model’s predictive utility and mechanistic coherence.
