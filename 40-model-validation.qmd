...
Similarly, parameter variations for CaMKII and NMDAR binding rates were explored to assess their impact on the model.  Specifically, a $k_\text{off}$ for the unbinding of CaMKII from the NR1 subunit of NMDARs was estimated by [@leonard2002Regulation], though to the best of our knowledge, we could not find estimated values for the GluN2B subunit, to which CaMKII binds more readily [@strack1998Autophosphorylationdependent; @leonard2002Regulation; @barria2005NMDA]. We therefore proceeded to use the parameters that were used for a previously validated model [@romangarcia2017Computationala], with a $k_{\text{D}}$ value of $1.5 \times 10^{-7} \text{M}$. We run the simulation by varying this $k_{\text{D}}$ by factor of 0.1 and 0.01.  The goal was to quantify how these parameter changes influenced model predictions, focusing on outputs such as CaMKII T286 phsphorylatuon and Cam binding because they are affected by binding indirectly so wanyed to check like that. 

After varying kd as described, we found a paper that recently has proposed a Kd of $k_{\text{D}}$ = $1.07 \times 10^{-7} \text{M}$. So, looking at how the results didnt vary that much and  with this new kd that matched our original one, we proiceeded with choosing a kd of $1.5 \times 10^{-7} \text{M}$. 

Given the known $k_{\text{D}}$ and $k_\text{off}$ we varied $k_\text{on}$ 

 A recent study proposed a $k_{\text{D}}$ = 107nM for CaMKII/GluN2B binding [@ozden2022CaMKIIb], but the absence of a known $k_\text{off}$ prompted us to investigate whether changing these parameters would significantly affect the model.
​
  by factors of 1, 0.1, and 0.01. The goal was to quantify how these parameter changes influenced model predictions, focusing on outputs such as CaMKII T286 phosphorylation,
---
...
also why did we choose t286 p and cam_ca4 to check these thigns? because they are affected by binding indirectly so wanyed to check like that. 
