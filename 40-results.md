...
directly enhances the affinity for CaM 

NMDAR binding keeps CaMKII in an open conformation independently of CaM, whereas T286P directly enhances the affinity for CaM. 

but both t286 o and nmdar keep camkii in open state why is only 286p the important bit? i think it has to do with the fact that when camkii is kept in the open state by nmdar, this is independent of cam. and not only that but nmdar binding promotes further t286p AND t306 which the latter is in direct competition for cam binding to camkii subunit . so with this logic, when we abolish t286p, cam binding decreases because this is the main opening mechanism that is allowing cam to bind; whereas nmdar binding keeps camkii open but is not the main mechanism by which cam is binding to camkii. soooo when we abolish nmdar camkii binding, the main mechanism for cam binding - t286 PHOSPHORylation - is intact, and therefore cam binding is intact. 

another interesting thing we see in the results of this model is the emergent CaM trapping behaviour. usually, cam trapping is defined or occurs because hosphorylation of CaMKII at Thr286 (or T286) increases its affinity for CaM, preventing its dissociation [REFERENCE]. this is not something that we have coded into the model, we do not specify an increased affinity rate for cam once camkii is t286, YET We see that Autophosphorylation at T286 promotes CaM binding, and that in fact when t286 phosphorylation is abolished, cam binding is decreased; whereas cam binding is not decreased for nmdar/camkii mutant likely because the latter mutant still retains t286p which is important for cam binding. 

DISCUSS THIS CONTRADICTION: NMDAR binding also promotes CaMKII autophosphorylation at T306 site
--
...
get into limitations. THIS NEEDS TO BE KEPT IN.
